...
execute if block ~ ~ ~ minecraft:crying_obsidian unless entity @e[type=minecraft:marker,tag=portal.end,distance=..0.1] positioned ~1 ~ ~ run function portal:portal_crying_obsidian/x_edge_right

# Locate side edge, skipping corner. (Why does this act differently from the other edge transitions??)
execute if block ~ ~1 ~ minecraft:crying_obsidian positioned ~ ~1 ~ run function portal:portal_crying_obsidian/x_edge_up
