# Made by Aelve
# Called by portal/functions/portal_<>/z_edge_up.mcfunction
# Checks top side of frame is complete (z axis).
...
execute if block ~ ~ ~ minecraft:dark_prismarine positioned ~ ~ ~-1 run function portal:portal_dark_prismarine/z_edge_left

# Locate side edge, skipping corner.
execute if block ~ ~ ~ minecraft:dark_prismarine if block ~ ~-1 ~-1 minecraft:dark_prismarine positioned ~ ~-1 ~-1 run function portal:portal_dark_prismarine/z_edge_down
