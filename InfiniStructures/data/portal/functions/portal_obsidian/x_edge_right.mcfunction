...
execute if block ~ ~ ~ minecraft:obsidian unless entity @e[type=minecraft:marker,tag=portal.end,distance=..0.1] positioned ~1 ~ ~ run function portal:portal_obsidian/x_edge_right

# Locate side edge, skipping corner. (Why does this act differently from the other edge transitions??)
execute if block ~ ~1 ~ minecraft:obsidian positioned ~ ~1 ~ run function portal:portal_obsidian/x_edge_up
